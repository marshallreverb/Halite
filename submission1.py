...
def get_target_direction(source, target):
        ...
        """
        return (ShipAction.EAST if target[0] > source[0] else ShipAction.WEST if target[0] < source[0] else None,
                ShipAction.SOUTH if target[1] < source[1] else ShipAction.NORTH if target[1] > source[1]else None)
...
def get_unsafe_moves(size, source, destination):
        ...
        x_cardinality, y_cardinality = get_target_direction(source, destination)
...
def req_dis(player,ship):
    ...
    for shipyard in player.shipyards:
        ...
        if dist > MAX_SHIPYARD_DIST and dist < 9:
             counter = counter + 1
...
def agent(obs, config):
    ...
    if len(me.shipyards) != 0 and board.step < 310 and me.halite >500:
        rd_sy = 0
        while rd_sy < len(me.shipyards)   :
            if  is_occupy_shipyard(me.ships,me.shipyards[rd_sy]) == False:
                me.shipyards[rd_sy].next_action = ShipyardAction.SPAWN
                break
            rd_sy = rd_sy+1
     

    if board.step > int(TOTALSTEPS/8) and len(me.shipyards) != 0 and len(me.shipyards) < MAX_SHIPYARD:
        for ship in me.ships :
            if req_dis(me,ship):
                ship.next_action = ShipAction.CONVERT
                break 
    elif len(me.ships) > 40 and len(me.shipyards) < MAX_SHIPYARD+2:
        for ship in me.ships :
            if req_dis(me,ship):
                ship.next_action = ShipAction.CONVERT
                break
